...
void generate_mrk_headers(Config *c, FILE *fp) {
    array *nodes = c->nodes;
...
void generate_mrk_sources(Config *c, FILE *fp) {
    int indent = 0;
    array *nodes = c->nodes;

    out("#include \"lib/print.h\"\n");
    out("#include \"generated/ast.h\"\n");
    out("#include \"generated/ccn_marking.h\"\n");
    out("#include \"core/internal_phase_functions.h\"\n");
    out("#include \"core/ast_memory.h\"\n");
    out("#include <stdio.h>\n");
}
...
void generate_marking_definitions(Config *c, FILE *fp) {
    generate_mrk_sources(c, fp);
}

// TODO: can optimize some if statemens away for example (->) is always true or when no range is specified.

static int generate_marking_traversal_children(Node *n, int indent, FILE *fp) {
...
static int generate_marking_traversal_attributes(Config *c, Node *n, int indent, FILE *fp) {
...
void generate_marking_traversal(Config *c, FILE *fp) {
    int indent = 0;
    out("#include \"generated/ast.h\"\n");
    out("#include \"generated/traversal-_CCN_MRK.h\"\n");
    out("#include \"generated/ccn_marking.h\"\n");
    ...
    out_field("void *none");
    out_struct_end();
    out_start_func("Info *_CCN_MRK_createinfo(void)");
    out_statement("return NULL");
    out_end_func();

    out_start_func("void _CCN_MRK_freeinfo(Info *info)");
    out_statement("mem_free(info)");
    out_end_func();
    for (int i = 0; i < array_size(c->nodes); ++i) {
        Node *n = array_get(c->nodes, i);
        out_start_func("void _CCN_MRK_%s(%s *node, Info *info)", n->id, n->id);
        //out_statement("printf(\"Printing adress of Node, \%\%p\\n\", node)");
        out_statement("mark_ast_memory_manager_entry(node)");

        //indent = generate_marking_traversal_children(n, indent, fp);
        //indent = generate_marking_traversal_attributes(c, n, indent, fp);
