// Hash: 3e39a0cd59728ac5797ddd72e5100164
#include <stdio.h>
#include "lib/print.h"
#include "generated/trav-VarDec.h"
// generated/trav-core.h is included by my header.
extern NodeType node_replacement_type;
extern void *node_replacement;

void _CCNtrav_BinOp(struct BinOp *node, struct Info *info);
void _CCNtrav_BoolConst(struct BoolConst *node, struct Info *info);
void _CCNtrav_Cast(struct Cast *node, struct Info *info);
void _CCNtrav_FloatConst(struct FloatConst *node, struct Info *info);
void _CCNtrav_FunCall(struct FunCall *node, struct Info *info);
void _CCNtrav_IntConst(struct IntConst *node, struct Info *info);
void _CCNtrav_MonOp(struct MonOp *node, struct Info *info);
void _CCNtrav_Ternary(struct Ternary *node, struct Info *info);
void _CCNtrav_Var(struct Var *node, struct Info *info);
void _CCNtrav_VarDec(struct VarDec *node, struct Info *info);

void CCNtrav_VarDec_expr(VarDec *node, struct Info *info) {
    node->expr = _CCNtrav_Expr(node->expr, info);
}

void CCNtrav_VarDec_next(VarDec *node, struct Info *info) {
    node->next = _CCNtrav_VarDec(node->next, info);
}

VarDec *_CCNtrav_VarDec(VarDec *node, struct Info *info) {
    if (!node) return node;
    switch (CCNtrav_current())) {
    case TRAV_Context:
        return Context_VarDec(node, info);;
    case TRAV_SplitInit:
        return SplitInit_VarDec(node, info);;
    case TRAV_Symbols:
        break;
    default:
        CCNtrav_VarDec_expr(node, info);
        CCNtrav_VarDec_next(node, info);
        break;
    }
    return node;
}

void replace_VarDec(struct VarDec *node) {
    if (node_replacement == NULL) {
        node_replacement_type = NT_VarDec;
        node_replacement = node;
    } else {
        print_user_error("traversal-driver", "replace_VarDec: Not making a node replacement, since another replacement function was already called previously.");
    }
}

